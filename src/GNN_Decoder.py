...
class GNN_Decoder:
    def __init__(self, params=None):
        ...
        def update_params(params_dict, input):
            for key in input:
                if key in params_dict:
                    ...
                    if isinstance(params_dict[key], dict) and isinstance(
                        input[key], dict
                    ):
                        update_params(params_dict[key], input[key])
                    elif not isinstance(params_dict[key], dict) and not isinstance(
                        input[key], dict
                    ):
        ...
        self.optimizer = torch.optim.Adam(
            self.model.parameters(), lr=p["model"]["initial_learning_rate"]
        )
    ...
    def load_best_weights(self):
        try:
            ...
            print(("The model has not yet been trained " "-- no best weights to load"))
    ...
    def train_with_data_buffer(
        ...
        def generate_buffer(buffer_size):
            ...
            for sampler in samplers:
                ...
                while len(stim_data_one_p) < (batch_size * buffer_size):
                    stim_data, observable_flips = sampler.sample(
                        shots=factor * batch_size * buffer_size,
                ...
                observable_flips_list.append(
                    observable_flips_one_p[: batch_size * buffer_size]
                )
            ...
            observable_flips_list = [
                val for tup in zip(*observable_flips_list) for val in tup
            ]
            ...
            for i in range(buffer_size):
                repeated_arguments.append(
                    (
                        stim_data_list[i * N_b : (i + 1) * N_b],
        ...
        def generate_test_batch(test_size):
            ...
            # syndromes. This number decreases with increasing p
            stim_data, observable_flips = sampler.sample(
                shots=test_size, separate_observables=True
            )
            # remove empty syndromes:
        ...
        def decode_test_batch(graph_batch):
            ...
            correct_predictions_nontrivial = count_correct_predictions_in_test_batch(
                graph_batch
            )
        ...
        def generate_and_decode_test_batch(test_size):
            ...
            test_accuracy = (
                correct_predictions_nontrivial + correct_predictions_trivial
            ) / test_size
            print(
                f"non-trivial syndromes: {len(graph_batch)}, trivial syndromes: {correct_predictions_trivial}"
            )
        ...
        print(
            f"Generating data with {cpu_count()} CPU cores, then moving it to device {device}."
        )
        print(
            (
                f"Starting training with {num_iterations} iteration(s)."
        ...
        previously_completed_samples = self.continuous_training_history[
            "num_samples_trained"
        ]
        if previously_completed_samples > 0:
            print(
                f"Cumulative # of training samples from previous runs: {previously_completed_samples:.2e}"
            )
        ...
        for ix in range(num_iterations):
            ...
            if (
                (num_iterations <= 100)
                or ((ix + 1) % training_print_spacing == 0)
                or (ix == 0)
            ):
                training_print_string = (
                    f"Iteration: {(ix+1):03d}\t"
                ...
                training_print_string += (
                    f"Cumulative # of training samples: "
        ...
        if save_to_file:
            ...
            self.save_attributes_to_file(prefix=save_file_prefix, suffix="")
...
def shuffle_buffer(buffer, repetitions):
    buffer = np.array(buffer).reshape((-1, repetitions))
